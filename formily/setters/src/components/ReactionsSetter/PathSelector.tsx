...
const transformDataSource = (node: TreeNode) => {
  ...
  const transformChildren = (children: TreeNode[], path = []) => {
    return children.reduce((buf, node) => {
      ...
      const title =
        node.props.title ||
        node.designerLocales?.['zh-cn']?.title ||
        node.props['x-component-props']?.title ||
        node.props.name ||
        node.designerProps.title;
      const label = operationType
        ? `${title}-${COperationTypeName[operationType]}`
        : title;
...
export const PathSelector: React.FC<IPathSelectorProps> = (props) => {
  ...
  designer.workbench.workspaces.forEach(item => {
    item.operation.tree.children && treeNode.children.push(...item.operation.tree.children);
  });

  const dataSource = transformDataSource(treeNode);
