  ...
  observer((props) => {
    ...
    const renderHistoryController = () => {
      ...
      return (
        <Radio.Group
          size="small"
          onChange={(e) =>
            isFn(history[e.target.value]) && history[e.target.value]()
          }
          style={{ marginRight: 20 }}
        >
          <Radio.Button value={'undo'} disabled={!history?.allowUndo}>
            <IconWidget infer="Undo" />
          </Radio.Button>
          <Radio.Button value={'redo'} disabled={!history?.allowRedo}>
    ...
    const renderCursorController = () => {
      ...
      return (
        <Radio.Group
          size="small"
          onChange={(e) => cursor.setType(e.target.value)}
          style={{ marginRight: 20 }}
          value={cursor.type}
        >
          <Radio.Button value={CursorType.Move}>
            <IconWidget infer="Move" />
          </Radio.Button>
          <Radio.Button value={CursorType.Selection}>
    ...
    const renderScreenTypeController = () => {
      ...
      return (
        <Radio.Group
          size="small"
          onChange={(e) => screen.setType(e.target.value)}
          style={{ marginRight: 20 }}
          value={screen.type}
        >
          <Radio.Button value={ScreenType.PC}>
            <IconWidget infer="PC" />
          </Radio.Button>
          <Radio.Button value={ScreenType.Mobile}>
            <IconWidget infer="Mobile" />
          </Radio.Button>
          <Radio.Button value={ScreenType.Responsive}>
