...
export const useResizeEffect = (engine: Engine) => {
  const findStartNodeHandler = (target: HTMLElement): ResizeData => {
    ...
    if (handler) {
      const type = handler.getAttribute(engine.props.nodeResizeHandlerAttrName) as Tdireation;
      if (type) {
  ...
  engine.subscribeTo(DragMoveEvent, (event) => {
    ...
    if (store.value) {
      ...
      const plusX =
        type === 'x-end' ? current.x > point.x : current.x < point.x;
      const plusY =
        type === 'y-end' ? current.y > point.y : current.y < point.y;
