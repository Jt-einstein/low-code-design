...
export class Workbench {
  ...
  setActiveWorkspace(workspace: Workspace | string) {
    if (isStr(workspace)) {
      const finded = this.findWorkspaceById(workspace);
      if (finded) {
        this.activeWorkspace = finded;
      }
  ...
  clearWorkspace() {
    this.eachWorkspace((item) => {
      this.removeWorkspace(item?.id);
    });
  }
}
